# This code is a modified version of pkoliveira/pyaiutils 'plots.py' code
import numpy as np
...
def plot_confusion_matrix(
    ...
    if cmap is None:
        ...
        'trunc({n},{a:.2f},{b:.2f})'.format(n=cmap.name, a=.35, b=.85),
        cmap(np.linspace(.35, .85, 100)))
...
def plot_auc_roc_multi_class(y_test, y_pred, class_names, visualize=False, save_path=None):
    ...
    if(len(y_pred.shape) == 1):
        y_pred = LabelEncoder(y_pred, np.unique(y_test))
        y_test = LabelEncoder(y_test, np.unique(y_test))
